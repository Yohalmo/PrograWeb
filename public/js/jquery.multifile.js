...
function JqueryMultiFileRemoved(restantes, container){ }
function JqueryMultiFileAdded(restantes, container){ }

;(function($, global, undefined){
  $.fn.multifile = function(containerOrAttributes, templateCb)
  {
    var container;
    let functionRemoveFile = JqueryMultiFileRemoved;
    let functionAddFile = JqueryMultiFileAdded;

    if (typeof containerOrAttributes == "object") {
      container = containerOrAttributes['container'];
      templateCb = containerOrAttributes['template'];
      functionRemoveFile = containerOrAttributes['onRemoveFile'] ?? JqueryMultiFileRemoved;
      functionAddFile = containerOrAttributes['onAddFile'] ?? functionAddFile;
    } else {
      ...
      , addInput = function(event)
        {
          ...
          templateCb($.fn.multifile.getFileObject(this))
            ...
            var restantes = $(container).children().length;
            functionAddFile(restantes, container);
        }
      , bindRemoveInput = function($input)
        {
          ...
          return function(event)
          {
            ...
            var restantes = $(container).children().length;
            functionRemoveFile(restantes, container);
